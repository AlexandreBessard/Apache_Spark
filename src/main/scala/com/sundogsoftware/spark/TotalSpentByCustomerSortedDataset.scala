...
object TotalSpentByCustomerSortedDataset {
  ...
  def main(args: Array[String]) {
    ...
    val customerDS = spark.read
      ...
      // 44,8602,37.19
      .csv("data/customer-orders.csv") // DataFrame
      .as[CustomerOrders] // DataSet

    val totalByCustomer = customerDS
      .groupBy("cust_id")
      // Aggregate functions together in a single column
      .agg(round(sum("amount_spent"), 2)
        .alias("total_spent"))

    val totalByCustomerSorted = totalByCustomer.sort("total_spent")
    // Take parameter as number of rows to show
    // Show the entire dataset and not only the first 20 rows as default.
