...
import org.apache.spark.sql.functions.{broadcast, current_timestamp, date_format}
...
object Test9 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dfWithWeekDay = df.withColumn("week_day_full", date_format(current_timestamp(), "EEEE"))
    ...
    val dfExtractedDay = df.withColumn("day_of_week", date_format(df("timestamp"), "EEEE"))

    // Example 3: Extracting the year from a timestamp column
    val dfExtractedYear = df.withColumn("year", date_format(df("timestamp"), "yyyy"))

    // Example 4: Adding a new column with a formatted date
    val dfFormattedDate = df.withColumn("formatted_date", date_format(df("timestamp"), "yyyy-MM-dd HH:mm:ss"))
