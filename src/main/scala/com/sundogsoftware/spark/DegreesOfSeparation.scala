...
object DegreesOfSeparation {
  ...
  // Find degres of seperation between Spider man and Adam
  ...
  var hitCounter:Option[LongAccumulator] = None // Telling i have a connection
  
  // Some custom data types 
  // BFSData contains an array of hero ID connections, the distance, and color.
  type BFSData = (Array[Int], Int, String) // String represents the color
  ...
  def createStartingRdd(sc:SparkContext): RDD[BFSNode] = {
    val inputFile = sc.textFile("data/Marvel-graph.txt") // RDD
    inputFile.map(convertToBFS) // Convert to BFS
  }
  ...
  def main(args: Array[String]) {
    ...
    // Spark context because we are going to use RDDs
